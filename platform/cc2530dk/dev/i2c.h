...
#define  SDA_1   st(SDA_OUT; SDA=1;)
#define  SDA_0   st(SDA_OUT; SDA=0;)
#define  SCL_1   st(SCL_OUT; SCL=1;)
#define  SCL_0   st(SCL_OUT; SCL=0;)

/* 写入指定设备 指定寄存器一个字节 */
extern unsigned char I2C_WRITE_BYTE(unsigned char i2c_addr, unsigned char reg_addr, unsigned char date);

/* 读取指定设备 指定寄存器的一个值。 */
extern unsigned char I2C_READ_BYTE(unsigned char i2c_addr, unsigned char reg_addr);
#endif